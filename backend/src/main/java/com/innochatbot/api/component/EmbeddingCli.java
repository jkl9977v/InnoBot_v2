...
public class EmbeddingCli implements CommandLineRunner {        // 텍스트 임베딩 기능을 수행한다.
    ...
    public void run(String... args) throws Exception {
        ...
        List<Path> changedFiles = fileScanner.getChangedPdfFiles("D:/InnoBot_v3/docs");
        
        for(Path filePath : changedFiles) {
        	try {
        		String fileId = fileScanner.getFileId(fileName, filePath);
        		...
        		fileScanner.updateFileHash(fileId, filePath);
		...
		/*
		 * // 1. docs 폴더 내 PDF 파일 순회 Path docsDir = Paths.get("D:/InnoBot_v3/docs"); //
		 * 문서파일 위치 경로 Files.walk(docsDir) .filter(p -> p.toString().endsWith(".pdf")) //
		 * .pdf 확장자만 선택 .forEach(filePath -> processPdf(filePath)); // 각 PDF 처리를 위한 파일경로
		 * 전달
		 */
